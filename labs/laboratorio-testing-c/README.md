...
> R:inicializa el carrito poniendo c->cantidad = 0.porque en C una variable local como Carrito c; no se inicializa automáticamente — el campo cantidad arranca con un valor indefinido (basura de memoria).
...
> R:NO.valor esperado 700. obtuvo 350.
...
> R:24.suma solo el precio unitario de cada producto, sin tener en cuenta cuántas unidades (cantidad) hay de ese producto.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Debería multiplicar el precio por la cantidad de cada item
...
> R:Falla.Devuelve 1 en vez de 0.
...
> R:Sí, las líneas 29-30, dentro de carrito_descuento. No se ejecutaron porque ningún test de tests/test_unitarios.c llama a esa función
...
> R:Un test unitario prueba una función aislada,Un test de integración, en cambio, verifica que varias funciones trabajen bien combinadas.El bug de carrito_total lo detectó primero un test unitario
...
> R:En C, los arrays no tienen verificación de límites.En Python o Java, los arrays sí tienen verificación de límites
